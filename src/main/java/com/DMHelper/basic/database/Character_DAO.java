...
public class Character_DAO {
    ...
    public static void save_character(Character_Sheet character) {
        String sql = "INSERT INTO saved_characters "
                + "(name, age, gender, background_story, personality_traits, ideals, bonds, flaws, race_name, class_name, current_level, experience_points, hp, current_hp, available_hit_dice, ac, "
    ...
    public static void load_all_characters() {
        ...
        try (Connection ignored = conn;
            ...
            while (rs.next()) {
                ...
                int savedAvailableHitDice = rs.getInt("available_hit_dice");
                loadedChar.set_available_hit_dice(savedAvailableHitDice < 0 ? loadedChar.max_hit_dice : savedAvailableHitDice);
    ...
    public static void update_character(Character_Sheet character) {
        ...
        String sql = "UPDATE saved_characters SET "
                ...
                + "hp = ?, current_hp = ?, available_hit_dice = ?, ac = ?, gold_pieces = ?, silver_pieces = ?, copper_pieces = ?, str = ?, dex = ?, con = ?, intel = ?, wis = ?, cha = ?, armor_name = ?, armor_type = ?, "
    ...
    private static void bind_character(PreparedStatement pstmt, Character_Sheet character) throws SQLException {
        ...
        pstmt.setInt(15, character.available_hit_dice);
        pstmt.setInt(16, character.ac);
        pstmt.setInt(17, character.gold_pieces);
        pstmt.setInt(18, character.silver_pieces);
        pstmt.setInt(19, character.copper_pieces);
        pstmt.setInt(20, character.stats.str);
        pstmt.setInt(21, character.stats.dex);
        pstmt.setInt(22, character.stats.con);
        pstmt.setInt(23, character.stats.intel);
        pstmt.setInt(24, character.stats.wis);
        pstmt.setInt(25, character.stats.cha);
        pstmt.setString(26, character.equipped_armor.armor_name);
        pstmt.setString(27, character.equipped_armor.armor_type);
        pstmt.setInt(28, character.equipped_armor.base_ac);
        pstmt.setInt(29, character.has_shield ? 1 : 0);
        pstmt.setString(30, Persistence_Util.encode_list(character.owned_equipment_keys));
        pstmt.setString(31, Persistence_Util.encode_int_map(character.inventory_item_counts));
        pstmt.setString(32, character.equipped_armor_key == null ? "" : character.equipped_armor_key);
        pstmt.setString(33, character.equipped_main_hand_key == null ? "" : character.equipped_main_hand_key);
        pstmt.setString(34, character.equipped_off_hand_key == null ? "" : character.equipped_off_hand_key);
        pstmt.setString(35, character.equipped_cloak_key == null ? "" : character.equipped_cloak_key);
        pstmt.setString(36, character.equipped_accessory_key == null ? "" : character.equipped_accessory_key);
        pstmt.setString(37, Persistence_Util.encode_list(character.job.skill_proficiencies));
        pstmt.setString(38, Persistence_Util.encode_list(character.job.feat_names));
        pstmt.setString(39, Persistence_Util.encode_list(character.advancement_notes));
        pstmt.setString(40, Persistence_Util.encode_map(character.job.export_class_state()));
        pstmt.setInt(41, character.job.used_asi_choices);
    }

    private static void bind_character_for_update(PreparedStatement pstmt, Character_Sheet character) throws SQLException {
        bind_character(pstmt, character);
        pstmt.setInt(42, character.database_id);
    }
    ...
    private static Character_Class build_job(String class_name) {
        ...
        if ("吟游诗人 (Bard)".equals(class_name)) {
